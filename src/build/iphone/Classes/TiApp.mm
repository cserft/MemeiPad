...
- (void)boot
{
	NSLog(@"[INFO] %@/%@ (%s.b7b9e78)",TI_APPLICATION_NAME,TI_APPLICATION_VERSION,TI_VERSION_STR);
...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kMemeiPadUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kMemeiPadUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
