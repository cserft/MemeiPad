...
{
	if (self = [super init]) {
		_memeipadRoot = [rootController retain];
		proxy = split_;
...
-(void)loadView
{
	[[NSNotificationCenter defaultCenter] removeObserver:_memeipadRoot name:UIDeviceOrientationDidChangeNotification object:nil];
	[super loadView];
}
...
{
	[_memeipadRoot refreshOrientationModesIfNeeded:oldCurrentWindow];
}
